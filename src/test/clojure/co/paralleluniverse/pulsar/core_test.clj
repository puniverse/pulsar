...
(facts "topics"
       ...
       (fact "When channel unsubscribes from topic then it stops receiving messages"
             (let [ch1 (channel)
                   ...
                   ch3 (channel)
                   topic (topic)
                   f1 (spawn-fiber #(list (rcv ch1 100 :ms) (rcv ch1 100 :ms) (rcv ch1 100 :ms)))
                   f2 (spawn-fiber #(list (rcv ch2 100 :ms) (rcv ch2 100 :ms) (rcv ch2 100 :ms)))
                   f3 (spawn-fiber #(list (rcv ch3 100 :ms) (rcv ch3 100 :ms) (rcv ch3 100 :ms)))]
               (subscribe topic ch1)
               (subscribe topic ch2)
               (subscribe topic ch3)
               (Thread/sleep 5)
